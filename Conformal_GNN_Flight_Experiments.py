...
class Dynamic_Network(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y = torch.tensor(self.classes[self.n * idx : self.n * (idx + 1)], dtype=torch.long)
...
for (method, GNN_model) in product(methods, GNN_models):
    ...
    for i in range(num_train_trans):
        ...
        for j in tqdm(range(num_permute_trans)):
            ...
            results[method][GNN_model]['Trans']['Accuracy']['All'].append(accuracy(best_output, data, test_mask))
            ...
            for t in range(T):
                ...
                results[method][GNN_model]['Trans']['Accuracy'][t].append(accuracy(best_output, data, test_mask_t))
...
for (method, GNN_model) in product(methods, GNN_models):
    ...
    for i in range(num_train_semi_ind):
        ...
        results[method][GNN_model]['Semi-Ind']['Accuracy']['All'].append(accuracy(best_output, data, test_mask))
        ...
        for t in range(T):
            ...
            results[method][GNN_model]['Semi-Ind']['Accuracy'][t].append(accuracy(best_output, data, test_mask_t))
