...
results_file = "results/school_with_sage.pkl"
...
class SGC(torch.nn.Module):
    def __init__(self, num_nodes, num_channels, num_classes, seed):
        super().__init__()
        torch.manual_seed(seed)
        self.conv1 = SGConv(num_nodes, num_channels, K=1)
        self.conv2 = SGConv(num_channels, num_classes, K=1)
...
for method, GNN_model in product(methods, GNN_models):
    ...
    for i in range(num_train_trans):
        ...
        if GNN_model == "GAT":
            model = GAT(data.num_nodes, num_channels_GAT, num_classes, seed=i)
        if GNN_model == "SAGE":
            model = GraphSAGE(data.num_nodes, num_channels_SAGE, num_classes, seed=i)
        if GNN_model == "SGC":
            model = SGC(data.num_nodes, num_channels_SGC, num_classes, seed=i)
        ...
        avg_coverage = np.mean(results[method][GNN_model]["Trans"]["Coverage"]["All"])
        print(f"Test coverage: {avg_coverage:0.3f}")
...
for method, GNN_model in product(methods, GNN_models):
    ...
    for i in range(num_train_semi_ind):
        ...
        avg_coverage = np.mean(
            results[method][GNN_model]["Semi-Ind"]["Coverage"]["All"]
        )
        print(f"Test coverage: {avg_coverage:0.3f}")
